...
const _formatPropertyContext = async function (changes, reqData, reqTarget) {
  for (const change of changes) {
    ...
    if (isAssociatedEntityProperty) {
      ...
      const keyConditions = (a.keys ?? []) ?.flatMap(k => [{ref: [a.name, k.ref[0]]}, '=', {ref: [k.$generatedFieldName], isManaged: true}]);
      const conditions = [...(a.on ?? []), ...keyConditions]
      const condition = conditions.reduce((agg, e, i) => {
        if (e === "=") {
          const targetProperty = [...conditions[i - 1].ref];
          targetProperty.shift();
          const sourceProperty = conditions[i + 1].ref.join(".");
          if(sourceProperty !== change.attribute && !conditions[i + 1].isManaged) {
          ...
          agg.changedFrom[targetProperty.join(".")] = change.valueChangedFrom;
          agg.changedTo[targetProperty.join(".")] = change.valueChangedTo;
        } return agg;
      }, {changedFrom: {}, changedTo: {}})
  
      const from = (change.modification === 'create' || condition.changedFrom.Id === '') ? '' : await cds.db.run(SELECT.one.from(a.target).where(condition.changedFrom));
      const to = (change.modification === 'delete' || condition.changedTo.Id === '') ? '' :  await cds.db.run(SELECT.one.from(a.target).where(condition.changedTo));
...
async function trackChangesForDiff(diff, req, that){
  ...
  if (
    ...
    await DELETE.from(`sap.changelog.ChangeLog`).where({entityKey: stringifyKey(entityKey)});
    return;
  }
  ...
  const isRelevantChange = c => (!isEmpty(c.valueChangedFrom) || !isEmpty(c.valueChangedTo)) && c.valueChangedFrom != c.valueChangedTo

  return {
    entity: dbEntity.name,
    entityKey: stringifyKey(entityKey),
    serviceEntity: target.name || target,
    changes: changes.filter(isRelevantChange).map((c) => ({
      ...c,
